...
├── tests/                       # 97 unit tests — data, features, model, evaluate, serving, bootstrap, pipeline, auth, state, and router coverage
├── .github/workflows/
...
Two adaptations from the source project were required, not optional, to make any of this work against this project's actual architecture: it assumed a local `data/processed/` parquet file and a live `Date` column to sort by for its time-ordered validation split; this project's data lives on S3 (`get_storage_options()`), and `build_features_rich()` deliberately keeps `Date` (unlike the lean `build_features()`, which drops it) both to sort before computing lag/rolling windows and so serving can merge new rows' computed features back out of a combined frame - it's explicitly excluded before training reaches RFECV/the model in `split_data_time_ordered()` (`src/data.py`), matching the source project's own explicit drop rather than relying only on the downstream `X.select_dtypes(include=[np.number])` filter to catch it.

The history-dependent features (lag/rolling/expanding Sales statistics) are only as good as how much continuous per-store history is actually loaded - a store with only a few weeks of data can't produce a meaningful `SalesLag28`. `featurize_rich` (`src/featurize_rich.py`) computes and logs a data-sufficiency diagnostic for exactly this - total rows, stores, date span, and the fraction of rows that are "cold start" for the 28-day window (necessarily `fillna(0)`, not a real value) - printing a warning above 10% cold-start, and `train_optimal.py` logs the same numbers directly onto the MLflow run (`data_*` params, `cold_start_fraction_lag28` metric) so a training run that underperforms is diagnosable from data volume alone, not just guessed at. `train_optimal.py` also fails loudly rather than training on a silently-degraded input if handed data with none of the expected history-dependent columns at all - e.g. if `featurize_rich` never ran, or `DATA_STORAGE_ROOT` pointed somewhere unexpected.
