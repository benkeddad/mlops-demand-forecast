...
- **A fifth `Deployment` + `LoadBalancer Service` pair for LocalStack itself** (`localstack/localstack:4.4.0`, plain community image, no auth token needed), pre-pulled directly into containerd by the k3s deploy scripts the same way Postgres/Redis are. k3s's built-in ServiceLB publishes its `LoadBalancer` Service on the node's own real IP - the same address `localstack_endpoint` (below) already points at - so nothing else needed to change for pods to keep reaching it.
...
| Script | What it does |
|---|---|
| `scripts\deploy_k3s_clean_wsl.bat` | Destructive full rebuild. Resets WSL2, starts a fresh K3s server, pre-pulls base images into containerd (Postgres, Redis, and LocalStack `4.4.0`), builds and imports all three custom images (via WSL2's own Docker Engine), deletes every prior K8s resource **including PVCs**, then runs `terraform init && terraform apply` (which also stands up LocalStack itself as an in-cluster Deployment/Service - see [Infrastructure as Code](#infrastructure-as-code)). Maps LocalStack's port to Windows `localhost` afterward (see below), so it must be run **as Administrator**. Use for a first deploy or a genuinely clean slate. |
| `scripts\deploy_k3s_reconcile_wsl.bat` | Non-destructive recovery. Checks whether K3s is already running before starting it, and re-pulls the LocalStack `4.4.0` image into containerd (a cheap no-op if it's already cached). Individually checks every ConfigMap, PVC, Service, and Deployment the stack needs, and only runs `terraform apply` if something's actually missing. Restarts only the Deployments that fail a `rollout status` check. Never touches PVCs or existing state. Also requires **Administrator**, for the same localhost port-mapping reason as its clean counterpart. |
...
All four scripts also run `scripts\setup_localstack_bucket.sh` early on - what it's checking differs by path. **On the K3s path, LocalStack itself is pinned to `localstack/localstack:4.4.0` and runs as a Terraform-managed in-cluster Deployment/Service** (see [Infrastructure as Code](#infrastructure-as-code)), started by `terraform apply` like every other backing service - not by `localstack start -d` or LocalStack Desktop. Getting it onto Windows `localhost` needs one more step, though: a K3s `LoadBalancer` Service is **not** automatically reachable via plain `127.0.0.1` the way a Docker container's published port is - k3s's ServiceLB binds it to the node's own real IP, not the loopback interface. Both K3s scripts account for this with a `netsh interface portproxy` rule mapping Windows `127.0.0.1:4566` straight to the node's real IP - the exact same mechanism the Compose scripts already use for their own ports, and the reason both K3s scripts now also require **Administrator** (see the table above). An earlier version of this mapping used `kubectl port-forward` instead; it worked but was flaky (silently drops the tunnel periodically), which is why it was replaced. **On the Compose path, LocalStack still runs as a standalone Docker container** the script detects/starts itself (the `localstack` CLI or an existing `localstack-main` container) - no extra port-mapping step needed there, since Compose's own `ports: "4566:4566"` publish is already directly visible on `localhost`. Either way, once LocalStack is reachable the script idempotently creates the two S3 **buckets** both DVC and MLflow need (`rossmann-mlops-dvc-store`, `rossmann-mlflow-artifacts`) — and that's *all* it creates. Neither DVC nor MLflow needs any "folder" (S3 prefix) pre-created inside those buckets: S3 has no real directory structure, a prefix simply starts existing the instant the first object is written under it, so `dvc push` and `mlflow.log_model()` create their own `dvc-store/...` / `mlflow-artifacts/...` layout automatically the first time each one actually runs.
...
On the K3s path, the LocalStack image is pulled automatically by the deploy scripts above - nothing to install by hand. To pull the exact same pinned version yourself (for the Compose path, or just to inspect/run it manually), install the AWS CLI inside WSL2 Ubuntu and pull the image via WSL2's own Docker Engine (not Windows - every script here runs against `wsl bash`):

```bash
pip3 install awscli
docker pull localstack/localstack:4.4.0
```

This is the plain **community** image, not `-pro` - no `LOCALSTACK_AUTH_TOKEN` or account/login required, since S3 is a free-tier service. One caveat carried over from earlier hands-on debugging: LocalStack's own persistence feature is *not* enabled here (see [Known Limitations](#known-limitations)) — recreating the LocalStack container/pod itself (not just restarting a deploy script) wipes both buckets, at which point `setup_localstack_bucket.sh` recreating two empty buckets is necessary but not sufficient; training has to actually run again to repopulate them.
...
Finally, install the AWS CLI — required, not optional, since MLflow and DVC both depend on a real S3 endpoint existing (see [The Automation Scripts](#the-automation-scripts) for exactly why skipping this makes training fail). LocalStack itself is pulled automatically by the K3s deploy scripts (pinned to `4.4.0`); install it yourself only if you're on the Compose path:
...
Either script opens five live log windows for the API, MLflow, Prefect, Postgres, and LocalStack when it finishes (all but LocalStack also re-establish a `kubectl port-forward` tunnel; LocalStack's port is mapped via the more stable `netsh portproxy` rule set up earlier in the script instead). Both scripts build images through WSL2's own Docker Engine; Docker Desktop only needs to be running so its dashboard can show you what's built/running — it doesn't do the building itself.
