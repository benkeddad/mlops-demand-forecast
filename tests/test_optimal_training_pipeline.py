...
# import _run_dvc, dvc_ingest, dvc_featurize, dvc_push` is bare too, so it
# resolves to the same already-loaded `training_pipeline` module either way.
from optimal_training_pipeline import dvc_train_optimal, ml_optimal_training_pipeline
...
def test_optimal_pipeline_reuses_the_fast_pipelines_ingest_featurize_push_tasks():
    # The only new task is dvc_train_optimal - ingest/featurize/push are the
    # exact same Prefect task objects imported from training_pipeline.py,
    # not reimplemented copies.
    import training_pipeline
    assert ml_optimal_training_pipeline.fn.__globals__["dvc_ingest"] is training_pipeline.dvc_ingest
    assert ml_optimal_training_pipeline.fn.__globals__["dvc_featurize"] is training_pipeline.dvc_featurize
